(function () {
  ...
  function renderDisclosure(extraction) {
    ...
    empty(list);
    var pairEligible = extraction.groups.filter(function (group) { return group.site && group.histology; });
    var runsPairCheck = pairEligible.length > 1;
    var fields = [
    ...
    extraction.groups.forEach(function (group) {
      var hasStagingInputs = group.site && group.histology;
      var histology = group.histology ? Number(group.histology.value) : null;
      var contentConfig = (window.__LRI_CATALOG__ || {}).content || {};
      var isHematopoietic = histology != null && histology >= Number(contentConfig.heme_histology_min);
      fields.forEach(function (definition) {
        var fieldName = definition[0];
        var field = group[definition[0]];
        if (!field) return;
        if (fieldName === "laterality" && !(hasStagingInputs && runsPairCheck)) return;
        if ((fieldName === "site" || fieldName === "year") && !hasStagingInputs) return;
        if (fieldName === "histology" && !(hasStagingInputs || (isHematopoietic && group.behavior))) return;
        if (fieldName === "behavior" && !(hasStagingInputs && (runsPairCheck || isHematopoietic || ["0", "1", "2", "3"].indexOf(field.value) !== -1)) && !(isHematopoietic && group.histology)) return;
        var item = document.createElement("li");
    ...
    return async function (method, url, body) {
      ...
      try {
        ...
        if (response.status >= 200 && response.status < 300) {
          ...
          if (raw) data = await new Response(raw).json();
        }
